  ...
  while (true) {
    const arbitrageResults = await Promise.all(
      AMOUNTS_TO_QUOTE.map(async (amount) => {
        ...
        const { total, splits } = quote;

        if (splits.length === 0) {
          console.error('Unexpected number of splits for amount', amount);
          return null;
        }
        ...
        if (splits.length === 1) {
          ...
          if (split.route.length === 1) {
            // Handle single-hop route
            console.error('Single-hop routes are currently not handled');
            return null;
          } else {
            // Prepare multihop_swap call
            const calldata = [
          ...
          // Prepare multi_multihop_swap call
          const calldata = [
            ...
            ...splits.reduce((memo, split) => {
              ...
              return memo.concat(routeCalldata);
            }, []),
          ];
        ...
        const calls = [transferCall, swapCall, clearProfitsCall];

        // Estimate the fee for the calls
        let estimatedFee;
        try {
          const feeEstimate = await ACCOUNT.estimateFee(calls);
          estimatedFee = feeEstimate.suggestedMaxFee;
        } catch (feeError) {
          console.error('Fee estimation failed for amount', amount, feeError);
          return null;
        }

        // Calculate net profit
        const grossProfit = BigInt(total) - amount;
    ...
    const profitableResults = arbitrageResults
      .filter((result) => result && result.profit > MIN_PROFIT)
      .sort((a, b) => Number(b.profit - a.profit))
      .slice(0, NUM_TOP_QUOTES_TO_ESTIMATE);

    if (profitableResults.length > 0) {
      console.log('Executing top arbitrage', profitableResults[0]);

      try {
        const { calls, estimatedFee } = profitableResults[0];
        const maxFee = estimatedFee * 2n; // Adjust as needed
